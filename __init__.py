...
class MyMpdControl(MycroftSkill):
    def __init__(self):
        super(MyMpdControl, self).__init__(name="My MPD single control")
    ...
    def on_settings_changed(self):
        self.host = self.settings.get('radio')
...
#deletes a given playlist and adds selected titles from database search    
    def play_from_database_search(self, playlist, pos):
        self.open_connection()
        mpcc.clear()
...
#Searching in current playlist
    def search_in_current_playlist(self, query):
        try:
            self.open_connection()
...
#Stored playlists functions
    ...
    def playlist_replace_and_play(self, playlist, pos):
        self.open_connection()
        list_playlist = mpcc.listplaylists()
...
#next function is deprecated but maybe reactivated sometime    
    def search_in_stored_playlists(self, query):
        fields = ['title', 'name', 'artist']
        result = ''
        result_raw = {}
        value_result = []
        pl_dict = self.merging_stored_lists()
...
#current function for searching in playlists
    def search_playlists_stored(self, query, merged_playlists):
        results = 0
...
#Searches in database
    def search_in_database_and_play(self, query, selection, pos):
        try:
            self.open_connection()
            mpcc.clear()
    ...
    def search_only_in_database(self, query, selection):
        try:
            self.open_connection()
...
#Intent handlers
    ...
    def handle_list_stored_playlists(self, message):
        ...
        if playlist == None:
            ...
            if pos_nr == None:
                ...
                self.playlist_replace_and_play(playlist, pos_nr)
            else:
                self.playlist_replace_and_play(playlist, pos_nr)
    ...
    def handle_search_all_playlists(self, message):
        ...
        query = message.data.get('query')
        merged_playlists = self.merging_stored_lists()
        self.search_playlists_stored(query, merged_playlists)

    @intent_handler('info_current_list.intent')
    def handle_speak_current_playlist(self, message):
        answer = self.speak_current_list()
        self.speak(answer)
    ...
    def handle_search_in_database(self, message):
        ...
        if query_correct == 'yes':
            ...
            if pos_nr == "": pos_nr = '0'
            self.search_in_database_and_play(query, selection, pos_nr)
        else:
            pass
    ...
    def handle_database_dialog(self, message):
        ...
        if selection == 'artist' or selection == 'title' or selection == 'album' or selection == 'genre':
            search_result = self.search_only_in_database(query, selection)
            ...
            if numbers_result != 0:
                ...
                if self.voc_match(title, 'nothing'):
                    ...
                    title = 0
                    self.play_from_database_search(search_result[0], title)
                else:
